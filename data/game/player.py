...
class Player:
    ...
    if repeat is true (selected):
        will play the same track over and over again, no other option matter
    if shuffle is true (selected):
        next track will be selected random from playlist, will not work if repeat is chosen
    if play_all is true (selected):
        play every track in playlist (folder), will not work if repeat or shuffle is chosen
    if loop is true (selected):
        will start playing from top of playlist when last song is played, will not work if repeat or shuffle is chosen,
        and will only work if play_all is true (selected)
    """
    def next(self):
        #if self.state == 3:
